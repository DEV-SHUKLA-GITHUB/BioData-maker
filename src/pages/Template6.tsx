...
import bg from "../../public/assets/bg4.jpg";
...
const Template6: React.FC<Template3Props> = ({ formData }) => {
        ...
        <div className="h-full text-white relative"
          ...
          {showHeader && (
            <div className="flex flex-row items-center gap-8 mb-8 rounded-lg p-6 shadow-lg backdrop-blur-sm">
              ...
              {/* Name and Basic Info */}
              <div className="flex flex-col text-white justify-center text-left">
            ...
            {items.map((item, index) => {
              ...
              return (
                <div key={`${section}-${key}-${index}`}>
                  ...
                  {/* Data Row */}
                  <div
                    className={`flex justify-between text-white items-start ${isMultiLine ? "" : "py-2"}`}
                    style={{
                      lineHeight: isMultiLine ? '1.5' : '.1rem',
                    }}
                  >
                    <span
                      className="font-semibold w-1/4  text-base mr-4 flex-shrink-0"
                      style={{ lineHeight: isMultiLine ? '1.5' : '.1rem' }}
                    >
                      {addSpaceBeforeCapitals(key)}:
                    </span>
                    <span
                      className=" w-1/2 text-base text-start"
                      style={{
          ...
          {/* Page Number */}
          <div className="absolute bottom-4 right-4 bg-white/90 px-3 py-1 rounded-full text-xs shadow-md">
            Page 1 of 1
          </div>
        </div>
      </div>
